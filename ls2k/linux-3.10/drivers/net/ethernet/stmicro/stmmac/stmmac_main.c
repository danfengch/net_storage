...
static struct ls2_gpio_chip gpio_chip0;
static inline void
...
static void storage_phy_status_to_fpga (struct phy_device *phydev, int speed)
{
    ...
    case 100:
        ...
        break;
    case 10:
        gpio0 = 0;gpio1 = 0;gpio2 = 1;
        break;
    case 0:
//        phydev->drv->config_init(phydev);
        gpio0 = 1;gpio1 = 0;gpio2 = 0;
        break;
    default:
        gpio0 = 1;gpio1 = 0;gpio2 = 1;
        break;
    ...
    name = (char *)dev_name(&phydev->dev);
    if(name[7] == '0')
        gpio18 = 0;
    else if(name[7] == '1')
        gpio18 = 1;
    else{
        pr_err("stmmac: storage_phy_status_to_fpga %s invalid.\n", name);
        return;
    }

    pr_info("%s - %s speed %d code : %d %d %d %d\n", dev_name(&phydev->dev), __func__, 
        speed, gpio18, gpio0, gpio1, gpio2);

    __set_level(&gpio_chip0, 19, 0);
    __set_level(&gpio_chip0, 18, gpio18);
    __set_level(&gpio_chip0, 0, gpio0);
    __set_level(&gpio_chip0, 1, gpio1);
    __set_level(&gpio_chip0, 2, gpio2);
    __set_level(&gpio_chip0, 19, 1);
//    myjif = jiffies + 2;
...
static void stmmac_adjust_link(struct net_device *dev)
{
	...
	if (phydev->link) {
...
#ifdef STORAGE_BOARD_MAC
            storage_phy_status_to_fpga(phydev, phydev->speed);
#endif
			new_state = 1;
			switch (phydev->speed) {
			case 1000:
		...
		priv->oldduplex = -1;
#ifdef STORAGE_BOARD_MAC
        storage_phy_status_to_fpga(phydev, 0);
#endif
	}
...
#if defined(STORAGE_BOARD_MAC)
    gpio_chip0.base = ioremap(0x1fe10500, 0x38);
	if (!gpio_chip0.base) {
        pr_err("stmmac: gpio_base ioremap failed\n");
		ret = -ENOMEM;
		goto err;
	}

    __set_direction(&gpio_chip0, 19, 0);
    __set_level(&gpio_chip0, 19, 0);
    __set_direction(&gpio_chip0, 18, 0);
    __set_direction(&gpio_chip0, 0, 0);
    __set_direction(&gpio_chip0, 1, 0);
    __set_direction(&gpio_chip0, 2, 0);  
#endif
