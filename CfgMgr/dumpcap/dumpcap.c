...
static GThread     *cfgmgrDumpCaptid;
...
//    fprintf(output, "  -e                       net number, 1/2\n");    
    fprintf(output, "\n");
#ifdef __linux__
...
//    msg *sendMsg = in, *recvMsg = out;
    ...
    do
    {
        ...
        i = (i - 1 + rb_data.num_files) % rb_data.num_files;
    }while(i != current_file_num);
...
cfgmgr_dumpcap_dialog(void* arg)
{
    ...
    g_log(LOG_DOMAIN_CAPTURE_CHILD, G_LOG_LEVEL_INFO, "Stopped cfgmgr_dumpcap_dialog %d.",
          net_number);
