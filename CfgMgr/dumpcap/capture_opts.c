...
//    printf("%s %d adapter_index = %ld, optarg_str_p :%s\n", __FILE__, __LINE__, adapter_index, optarg_str_p);
    capture_opts->default_options.net_number = atoi(&optarg_str_p[3]);
    printf("net_number = %d\n", capture_opts->default_options.net_number);
    if (p != NULL && *p == '\0') {
        if (adapter_index < 0) {
...
capture_opts_add_opt(capture_options *capture_opts, int opt, const char *optarg_str_p, gboolean *start_capture)
{
    ...
    case 'i':        /* Use interface x */
        ...
        break;
//    case 'Q':
//        capture_opts->default_options.net_number = atoi(optarg_str_p);
//        printf("optarg_str_p :%s, net_number = %d\n", optarg_str_p, capture_opts->default_options.net_number);
